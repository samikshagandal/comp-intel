...
export default function SalariesPage() {
  ...
  const fetchSalaries = async (dir?: 'asc' | 'desc') => {
    setLoading(true)
      ...
      <div className="max-w-7xl mx-auto px-8 py-10">
        ...
        <div className="flex items-center justify-between mb-8">
          <div>
            ...
            <p className="text-gray-600 text-sm mt-1 font-medium">{salaries.length} entries · sorted by total compensation</p>          </div>
          <div className="flex gap-2">
            {['google', 'amazon', 'meta'].map(co => (
              <button key={co} onClick={() => { setFilters(p => ({ ...p, company: co })); fetchSalaries() }}
          ...
          <div className="grid grid-cols-4 gap-3 mb-3">
            {(['company', 'role', 'level', 'location'] as const).map(f => (
              <input key={f}
                ...
                className="border-2 border-gray-200 rounded-xl px-4 py-2.5 text-sm placeholder-gray-500 focus:outline-none focus:border-violet-400 transition-all bg-white text-gray-800"                value={filters[f]}
        ...
        {loading ? (
          <div className="text-center py-32 text-gray-300 text-lg">Loading...</div>
        ) : salaries.length === 0 ? (
